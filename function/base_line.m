...
for m=1:7
    dim=8*2^(m-1);
    [test_features_pca,query_crop_features_pca]=pca_whitening(train_features_normalize,test_features_normalize,query_features_normalize,dim);
    if  ~strcmpi(test_set,'holidays')
        [test_features_pca,query_nocrop_features_pca]=pca_whitening(train_features_normalize,test_features_normalize,query_nocrop_features_normalize,dim);
    end
    dist=pdist2(test_features_pca,query_crop_features_pca,'seuclidean');%计算L2距离
    [~, ranks] = sort(dist, 'ascend');
    [sum_cropnorpca_map,~] = compute_map (ranks, gnd);

    dist=pdist2(test_features,query_features,'euclidean');%计算L2距离
    [~, ranks] = sort(dist, 'ascend');
    [sum_crop_map,~] = compute_map (ranks, gnd);

    dist=pdist2(test_features_normalize,query_features_normalize,'seuclidean');%计算L2距离
    [~, ranks] = sort(dist, 'ascend');
    [sum_cropnor_map,~] = compute_map (ranks, gnd);

    if  ~strcmpi(test_set,'holidays')
        dist=pdist2(test_features,query_nocrop_features,'seuclidean');%计算L2距离
        [~, ranks] = sort(dist, 'ascend');
        [sum_nocrop_map,~] = compute_map (ranks, gnd);

        dist=pdist2(test_features_normalize,query_nocrop_features_normalize,'seuclidean');%计算L2距离
        [~, ranks] = sort(dist, 'ascend');
        [sum_nocrop_nor_map,~] = compute_map (ranks, gnd);

        dist=pdist2(test_features_pca,query_nocrop_features_pca,'seuclidean');%计算L2距离
        [~, ranks] = sort(dist, 'ascend');
